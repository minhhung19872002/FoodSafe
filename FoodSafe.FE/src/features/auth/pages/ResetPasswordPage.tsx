...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    let active = true
    authApi.verifyPasswordResetToken(userId, resetToken)
  ...
  const mutation = useMutation({
    ...
    onError: () => message.error('Không thể đặt lại mật khẩu. Liên kết có thể đã hết hạn.'),
  })

  return (
    <div style={pageStyle}>
      <Card style={{ width: 460 }}>
        {verification === 'checking' && <Spin tip="Đang xác minh liên kết..." />}
        {verification === 'invalid' && (
          <Result
            status="error"
            title="Liên kết không hợp lệ hoặc đã hết hạn"
            extra={<Button onClick={() => navigate('/account/forgot-password')}>Gửi yêu cầu mới</Button>}
          />
        )}
