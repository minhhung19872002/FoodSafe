...
export default function PublicAdRegistrationLookupPage() {
  const [number, setNumber] = useState("");
  const [result, setResult] = useState<PublicAdRegistration>();
  ...
  const lookup = async () => {
    ...
    setResult(undefined);
    try {
      setResult(await publicAdRegistrationApi.lookup(number.trim()));
    } catch {
      setError("Không tìm thấy đăng ký quảng cáo.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main style={{ maxWidth: 900, margin: "64px auto", padding: "0 24px" }}>
      <Typography.Title level={2}>
        Tra cứu đăng ký quảng cáo thực phẩm
      </Typography.Title>
      ...
      {result && (
        <Card style={{ marginTop: 24 }}>
          <Descriptions bordered column={1}>
            ...
            <Descriptions.Item label="Nội dung quảng cáo">
