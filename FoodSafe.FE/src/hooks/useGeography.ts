...
const keys = {
  ...
  provinces: (activeOnly: boolean) => [...keys.all, 'provinces', activeOnly] as const,
...
export function useDistricts(provinceId: string, activeOnly = true) {
  return useQuery({
    ...
    queryFn: () => getGeographicItems<DistrictItem>(
      'districts',
      { provinceId, activeOnly },
    ),
...
export function useCommunes(districtId: string, activeOnly = true) {
  return useQuery({
    ...
    queryFn: () => getGeographicItems<CommuneItem>(
      'communes',
