...
[RemoteService(false)]
public class ExportFoodCertificateExcelAppService :
    ApplicationService,
    IExportFoodCertificateExcelAppService
{
    private const int PageSize = 1000;
    private const int MaximumRows = 50_000;
    private readonly IExportFoodCertificateAppService _certificates;

    public ExportFoodCertificateExcelAppService(
        IExportFoodCertificateAppService certificates)
    {
    ...
    {
        var rows = new List<ExportFoodCertificateDto>();
        long total;
        do
        {
            var page = await _certificates.GetListAsync(
                new ExportFoodCertificateListInput
                {
                    Filter = input.Filter,
            ...
            if (total > MaximumRows)
                throw new BusinessException(
                        "FoodSafe:ExportFoodCertificateExport:TooLarge")
        ...
        return new ExcelDownloadDto
        {
            ...
            FileName =
                $"giay-chung-nhan-xuat-khau-{Clock.Now:yyyyMMdd-HHmmss}.xlsx"
        };
    }

    private static byte[] CreateWorkbook(
        IReadOnlyList<ExportFoodCertificateDto> rows)
    {
        using var workbook = new XLWorkbook();
        var sheet = workbook.Worksheets.Add("GCN Xuất khẩu");
        var headers = new[]
        {
            "Cơ sở", "Số GCN", "Ngày cấp", "Sản phẩm",
            "Quốc gia đích", "Số lô", "Số lượng", "Đơn vị",
            "Ngày hết hạn", "Trạng thái", "Số ngày còn lại",
            "Lý do thu hồi", "Ghi chú"
        };
        ...
        foreach (var item in rows)
        {
            ...
            sheet.Cell(rowNumber, 5).Value =
                item.DestinationCountryName ?? string.Empty;
            sheet.Cell(rowNumber, 6).Value =
                item.LotNumber ?? string.Empty;
            if (item.Quantity.HasValue)
                sheet.Cell(rowNumber, 7).Value = (double)item.Quantity.Value;
            ...
            if (item.DaysUntilExpiry.HasValue)
                sheet.Cell(rowNumber, 11).Value = item.DaysUntilExpiry.Value;
