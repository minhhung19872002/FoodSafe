...
public class CfsCertificateAppService :
    ApplicationService,
    ICfsCertificateAppService
{
    private readonly IRepository<CfsCertificate, Guid> _registrations;
    ...
    public CfsCertificateAppService(
        IRepository<CfsCertificate, Guid> registrations,
    ...
    public async Task<PagedResultDto<CfsCertificateDto>> GetListAsync(
    ...
    public async Task<CfsCertificateDto> GetAsync(Guid id)
    {
    ...
    [Authorize(FoodSafePermissions.Licensing.CfsCertificates.Create)]
    public async Task<CfsCertificateDto> CreateAsync(
        UpsertCfsCertificateDto input)
    {
        ...
        await EnsureProductAsync(
            ...
            business,
            DataScopeOperation.Create);
        await EnsureCountryAsync(input.DestinationCountryId);
        await EnsureUniqueNumberAsync(input.CertificateNumber, null);

        var id = GuidGenerator.Create();
        var registration = CfsCertificate.Create(
            id,
            business.Id,
    ...
    [Authorize(FoodSafePermissions.Licensing.CfsCertificates.Edit)]
    public async Task<CfsCertificateDto> UpdateAsync(
        Guid id,
        UpdateCfsCertificateDto input)
    {
        ...
        if (input.BusinessId != registration.BusinessId)
            throw new BusinessException(
                FoodSafeDomainErrorCodes.CfsCertificate.ProductMismatch);
        var business = await GetScopedBusinessAsync(
    ...
    [Authorize(FoodSafePermissions.Licensing.CfsCertificates.Delete)]
    ...
    [Authorize(FoodSafePermissions.Licensing.CfsCertificates.Edit)]
    public async Task<CfsCertificateDto> RevokeAsync(
        Guid id,
    ...
    private async Task<IQueryable<CfsCertificate>> ScopedQueryAsync(
    ...
    private async Task<CfsCertificate> GetScopedAsync(
        Guid id,
        DataScopeOperation operation)
    {
        ...
        return await AsyncExecuter.FirstOrDefaultAsync(
               ...
               ?? throw new AbpAuthorizationException(
                   "The CFS certificate is outside the current user's data scope.");
    }
    ...
    private async Task EnsureCountryAsync(Guid countryId)
    {
        ...
        if (!await AsyncExecuter.AnyAsync(
                ...
                _cancellationTokens.Token))
            throw new BusinessException(
                FoodSafeDomainErrorCodes.CfsCertificate.CountryNotFound);
    }
    ...
    private async Task<List<CfsCertificateDto>> ToDtosAsync(
        IReadOnlyCollection<CfsCertificate> registrations,
        DateTime today)
    {
        ...
        var countryIds = registrations.Select(x => x.DestinationCountryId)
            .Distinct().ToArray();
        ...
        {
            var dto = ObjectMapper.Map<CfsCertificate,
                CfsCertificateDto>(x);
            ...
            dto.LinkedProductName = x.ProductId.HasValue
                ...
                : null;
            dto.DestinationCountryName =
                countries.GetValueOrDefault(x.DestinationCountryId) ??
                string.Empty;
            return dto;
        }).ToList();
    }
    ...
    private static IOrderedQueryable<CfsCertificate> ApplySorting(
        IQueryable<CfsCertificate> query,
        string? sorting)
    {
    ...
    private static IQueryable<CfsCertificate> ApplyStatusFilter(
        IQueryable<CfsCertificate> query,
        ...
        status switch
        {
            ...
            _ => throw new UserFriendlyException(
                "Trạng thái giấy đăng ký không hợp lệ.")
        };
}
