...
public class GeographicCatalogAppService : ApplicationService, IGeographicCatalogAppService
{
    ...
    public async Task<ListResultDto<CommuneDto>> GetCommunesByProvinceAsync(Guid provinceId, bool activeOnly = true)
    {
        var query = (await _communes.GetQueryableAsync())
            .Where(c => c.ProvinceId == provinceId);
        if (activeOnly) query = query.Where(c => c.IsActive);
        var items = await AsyncExecuter.ToListAsync(query.OrderBy(c => c.SortOrder).ThenBy(c => c.Name));
    ...
    public async Task DeleteProvinceAsync(Guid id)
    {
        ...
        if (await AsyncExecuter.AnyAsync(
                communes.Where(x => x.ProvinceId == id),
    ...
    public async Task<CommuneDto> CreateCommuneAsync(UpsertCommuneDto input)
    {
        ...
        var item = Commune.Create(GuidGenerator.Create(), input.Code, input.Name, input.ProvinceId, input.Type, input.SortOrder);
