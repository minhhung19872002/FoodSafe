...
| P1-1b | Attachment upload/download — inspection + 6 licensing + products | `attachmentRoundTrip` | 🔶 inspection DONE (`inspection-attachments.spec.ts` 2/2 — FR-27-08/09, FR-28-05): plan + result upload→**reload-persist**→download(`%PDF`)→delete, real ClamAV+MinIO, no interception. Products + 6 licensing modules **already** covered by existing `businesses`/`self-declarations`/`advertisement-registrations`/`eligibility-certificates`/`cfs-certificates`/`export-food-certificates` full-cycle specs. **`/documents` (admin-doc) attachment now DONE (`9ca9a7b`)** — `documents-attachments.spec.ts` 1/1 green (4.0s): real `/documents` modal round-trip (upload real PDF → ClamAV → MinIO → PostgreSQL → row with real "N KB" size + valid date → reload-persist → "%PDF" download → delete), no interception. **Fixed a real FE defect**: the modal read `sizeBytes`/`creationTime` (absent from `FileAttachmentDto` → NaN KB / Invalid Date) instead of `fileSize`/`uploadTime`; the BE endpoint already existed via the shared `InspectionAttachmentAppServiceBase`. **P1-1b fully closed.** |
...
| P1-1g | Concrete-artifact exports: administrative-document list Excel + per-document print (FR-38-07); statistics business-breakdown Excel (FR-40-07) | live-doc API seed | ✅ DONE (`f3b6920`) — 3/3 green (5.1s), real backend, no interception. **Documents** (`documents-export-print.spec.ts` 2/2): the live document list is normally empty, so one document is **seeded through the real authenticated admin API** (`POST /api/v1/app/administrative-document`, real cookie+antiforgery → ASP.NET → EF → PostgreSQL) after a real UI login; then at `/documents` the list **"Xuất Excel"** downloads a **non-empty OpenXML (PK) workbook** and the per-row **print** button opens the **real formatted print window** (`window.open` + `document.write`, asserted to render số văn bản + tiêu đề); seeded rows are deleted in `finally`. **Statistics** (`excel-exports.spec.ts` +1, FR-40-07): the **"Cơ sở SXKD"** tab's "Xuất Excel" (`GET /statistics/excel/business-breakdown`) downloads a non-empty PK workbook via the real UI. Closes the strongest remaining concrete-artifact export gaps in doc 73 |
| P1-1h | Risk-analysis publish (FR-36-07) + public portal exposure (FR-36-08) | anon-context re-check | ✅ DONE (`720d01a`) — `risk-analysis-publish.spec.ts` 1/1 green (4.1s), real backend, no interception. A **Draft** risk analysis is seeded via the real authenticated admin API (`POST /api/v1/app/risk-analysis`, Status=Draft) after a real UI login; then **FR-36-07** — at `/risk-analysis` the officer clicks the real **"Xuất bản"** button + Popconfirm → status Tag flips **Nháp → Đã xuất bản** and **persists across `page.reload()`** (`POST /risk-analysis/{id}/publish` sets Status=Published + IsPublic=true atomically); **FR-36-08** — from a **fresh anonymous browser context** (no admin session) `/tin-tuc` → tab "Phân tích nguy cơ" now lists the analysis (`GET /api/v1/public/risk-analyses`, `[AllowAnonymous]`), proving the publish exposed it publicly end-to-end. Cleanup: published rows are Draft-only-deletable, so the E2E-RISK row is soft-deleted via SQL out-of-band (0 residual live rows confirmed). **Note:** FR-36-08's "PDF" is a client-side `window.print()` of formatted HTML — there is no server PDF endpoint for risk-analysis, so that facet is browser-print, not a server artifact (recorded honestly in doc 73) |
| P1-1i | Per-business detail drawer related-record tabs (FR-19-11..16) | data-driven discovery | ✅ DONE (`2d010a8`) — `business-detail-tabs.spec.ts` 1/1 green (2.9s), real backend, no interception. To avoid brittle hard-coded IDs the test **discovers** a real business that owns related data by reading the real self-declaration list (`GET /api/v1/app/self-declaration?MaxResultCount=1` → reuse `businessId`/`businessName` from the DTO), then opens that business's **"Hồ sơ cơ sở"** drawer through the real browser (`/businesses` → search → "Hồ sơ …" button). All **5 tabs** each fire their **business-scoped** GET with `businessId=` asserted in the URL, returning 200: `self-declaration`, `product-registration`, `advertisement-registration`, `eligibility-certificate`, `inspection-result`; the data-bearing **"Tự công bố"** tab additionally shows ≥1 real row. Proves the drawer wires each tab to the correct org/business-scoped endpoint (not a render-only check) |
| P1-1j | Dashboard per-unit report-compliance widget (FR-39-03 ATTP work-report tracking, FR-39-04 Action-Month tracking) | baseline-delta seed | ✅ DONE (`3f0cf1b`) — `dashboard-report-compliance.spec.ts` 1/1 green (2.6s), real backend, no interception. Proves the widget's numbers are **driven by real report state**, not hard-coded: (1) read the real `GET /api/v1/app/dashboard/report-compliance` for a per-org **baseline**; (2) **seed + submit** one ATTP work report (`POST /atp-work-report` → Draft, then `/submit` → Submitted) and one Action-Month report (`/action-month-report`) for the officer's org through the **real workflow** (session cookie + antiforgery → ASP.NET → domain state machine → PostgreSQL); (3) re-read compliance and assert the seeded org shows **+1** for both (baseline+1); (4) the real `/dashboard` widget renders that org's **"BC công tác ATTP"** (`1/2`, FR-39-03) and **"BC Tháng hành động"** (`1/1`, FR-39-04) progress cells, **persisting across `page.reload()`**. Teardown walks the real workflow back (Return → ReturnToDraft → Delete) — verified 0 live non-draft residue; the baseline-delta keeps the test re-run-safe regardless. FR-39-09 (chart PNG download) now closed under P1-1m |
| P1-1k | NDTP roll-up aggregation "Tổng hợp từ báo cáo tuyến dưới" (FR-33-02) | baseline-delta + second-user context | ✅ DONE (`714e318`) — `ndtp-rollup-aggregation.spec.ts` 1/1 green (4.6s), real backend, no interception. Proves the higher-tier report's aggregated figures come from **real lower-tier state**: (1) read the real `GET /api/v1/app/report-calculation/ndtp-aggregation` per-period **baseline** (period from a per-run stamp to dodge the `idx_ndtp_org_period` unique-index residue collision); (2) as a **lower-tier** user (`district.staff@foodsafe.local`, second browser context) seed a report through the **real workflow** — create Draft → `PUT /{id}/stats` (real figures) → `POST /{id}/submit`; (3) re-read aggregation and assert `reportCount` + **all 8 stat fields** = **baseline + child**; (4) **org-isolation guard** — province draft org ≠ child org; (5) real browser at `/reporting`: filter by year (AntD InputNumber via `pressSequentially`), open draft editor, click **"Tổng hợp từ báo cáo tuyến dưới"** → `GET …/ndtp-aggregation` 200, success message `Đã tổng hợp từ {n} báo cáo tuyến dưới`, all 8 form fields populated; (6) **Save** persists across `page.reload()`. Teardown walks the child back (Return → ReturnToDraft → Delete) + deletes the province draft; baseline-delta keeps it re-run-safe. FR-34-10 (ATTP work auto-aggregation) is the analogous remaining item |
| P1-1l | ATTP work-report auto-aggregation "Tự tính số liệu từ hệ thống" (FR-34-10) | baseline-delta seed | ✅ DONE (`7d38721`) — `atp-work-auto-aggregation.spec.ts` 1/1 green (4.2s), real backend, no interception. Proves the ATTP report's auto-filled figures come from **real live system data**, not hard-coded: (1) read the real `GET /api/v1/app/report-calculation/atp-work-stats?PeriodType=2&PeriodYear=2026` **baseline**; (2) seed **one** business over real authenticated HTTP (CreationTime = now → falls in the 2026 annual period), re-read and assert `totalBusinesses` **and** `newBusinesses` each **baseline+1** and non-zero; (3) seed a Draft ATTP report, open it in the real browser at `/reporting`, click **"Tự tính số liệu từ hệ thống"** → `GET …/atp-work-stats` 200, success message `Đã tự tính số liệu từ dữ liệu hệ thống`, form `#totalBusinesses`/`#newBusinesses`/`#businessesInspected` populated with **EXACTLY** the endpoint response; (4) **Save** persists across `page.reload()`. Teardown deletes the Draft report + business; baseline-delta keeps it re-run-safe. Closes the last FR-34 auto-aggregation gap (FR-34-08 formatted view remains a low-priority IMPLEMENTED_NOT_VERIFIED view-only item) |
| P1-1m | Statistics chart PNG download (FR-39-09) | real-download magic-byte assert | ✅ DONE (`de06374`) — `statistics-chart-download.spec.ts` 1/1 green (1.7s), real backend, no interception. At `/statistics` the test clicks both time-series chart download buttons and asserts real PNG downloads (8-byte PNG signature + year-stamped filenames `thanh-kiem-tra-<year>.png` / `ngo-doc-thuc-pham-<year>.png`). **Fixed a real production defect** found via in-browser diagnosis: `downloadChartAsPng` fed the intermediate SVG to `<img>` through a `blob:` URL, which the app CSP (`img-src 'self' data: https://*.tile.openstreetmap.org` — no `blob:`) rejected before load, so rasterization rejected and no PNG was ever produced — the buttons silently failed in the real app. Switched the SVG source to a CSP-allowed `data:` URI (`encodeURIComponent` preserves the Vietnamese labels); no CSP change required. Confirmed zero CSP console errors after the fix |
