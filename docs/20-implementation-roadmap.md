...
Status: STT 21 is complete. Self-declarations now have scoped CRUD, effective
expiry filtering and 30/60/90-day alerts, terminal revocation with actor,
time and reason, retained-history number uniqueness, secure shared
attachments, Excel export, a permission-aware Vietnamese SPA, and
authenticated Docker E2E. STT 22–26 remain.
...
| Backend tests | Pass, 114 tests including PostgreSQL migration/constraint integration, business/product/self-declaration rules, Excel contracts, attachment validation/authorization, current-user permission projection, request contracts, password policy, CAPTCHA behavior and root-promotion authorization; coverage remains incomplete |
| Frontend strict production build | Pass |
| Frontend tests | Pass, 35 Vitest tests plus authenticated Playwright catalog, facility/product and self-declaration lifecycles; browser coverage includes Excel, clean/infected files, revocation and retained official-number identity |
| EF migrations | Pass; typed-owner attachment and self-declaration migrations apply on PostgreSQL through the one-shot migrator |
