...
Per the executed matrix (doc 73 + addenda), reconciled against this session's run: all software-assessable requirement groups carry passing real-stack evidence; the only remaining **MISSING** rows are **INT-01** (Bộ Y tế connectivity — external endpoint) and **INT-02** (TT 31/2026 + NĐ 37/2026 per-partner mapping — blocked on the external partner schema). **FR-50-05 is no longer outstanding**: both the published partner contract (`0776230`) and the in-app specification-management feature (`5bc0d86`, F-019g — upload/validate/version/publish/anonymous download, `e2e/api-specification-management.spec.ts` 4/4) are delivered and verified. Residual `IMPLEMENTED_NOT_VERIFIED` items are the four minor view-layer conveniences of doc 07 E-1 (audit-log detail drawer, profile edit/avatar, two formatted report views, further breakdown exports) — UAT-smoke material, not gate material.
...
| **M-4 / FR-50-05** | Partner-facing machine-readable API specification — confirmed absent (G10). **RESOLVED 2026-07-28 (post-gate, at `0776230`)**: `docs/integration/` spec + OpenAPI 3.0.3 + onboarding guide + examples published from the committed implementation; Redocly lint valid; executable contract test `e2e/partner-openapi-contract.spec.ts` 1/1 against the running stack (all operations, all 7 segments, all 10 error codes, schema-validated bodies). **Extended 2026-07-28 (at `5bc0d86`)**: FR-50-05 is now also a shipped in-app feature — admin upload/validate (JSON+YAML via `Microsoft.OpenApi.Readers`)/version/publish/unpublish + anonymous partner download, verified end-to-end by `e2e/api-specification-management.spec.ts` 4/4 (no interception); feature F-019g VERIFIED. FR-50-05 → PASS_WITH_EXECUTABLE_SPEC_EVIDENCE (doc 01, regression log) | **Documentation issue — RESOLVED (+ feature shipped)** | Surface is stable since `52d35c1`; small, mechanical |
