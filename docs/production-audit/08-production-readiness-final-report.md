...
But **production go-live is blocked by the deployment, database-operations, and security layers**, not the features. The production stack **physically cannot be built** (missing `Dockerfile.prod`), there is **no backup or restore capability**, **credentials committed to git history have not been rotated**, and there are **unmitigated SSRF, CAPTCHA-bypass, stored-XSS, and known-vulnerable-dependency** issues. Any one of the first three is an independent hard blocker. Shipping now would put a Level-2 government information system into production with no disaster recovery and a physically unbuildable release artifact.
