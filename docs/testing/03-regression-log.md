...
- **Details**: New E2E test `reporting-verification.spec.ts` test 7 (sorting + duplicate-period) covers L1 and L2. `ReportNameEnricher` batch-resolves org + user names via `IReportActorsDto` interface. `EnsurePeriodIsFreeAsync` added to all 3 report AppServices with dedicated `FoodSafe:Report:0010` error code. `ReportCalculationAppService.TotalAffected` now sums `PoisoningCaseCount + sum(incident.AffectedCount)`.

### 2026-07-28 — F-014 food-poisoning production-readiness hardening (incl. empty-record acceptance + code reuse)

- **Cause**: Full LOGIC/UI/UX audit of `/food-poisoning` (real browser + API probes + code review; F-014 was silently DIRTY — `4a4af68` had touched its page/queries/types after the `8be91bc` stamp). Found: (1) **server accepted completely empty cases/incidents** (`POST {}` → 200; only `ReportDate` was required anywhere) → `[Required]` VictimName/LocationDescription (case) + OccurrenceDate/LocationDescription (incident), `[Range]` on VictimAge and incident statistics; (2) **COUNT-based code generation reissued codes after delete-recreate** (soft-delete shrinks the count) and raced → max-suffix scan incl. soft-deleted + filtered unique indexes `uq_fpc_org_code`/`uq_fpi_org_code` (migration `20260728124853`); (3) case edit wiped `incidentId` and UI had no way to link cases to incidents (CaseCount dead at 0) → link select + preserve-on-edit; (4) error-report lookups threw wrong NotFound codes → `0007`, no status guards on Acknowledge/MarkCorrected → `0008`; (5) 10 hardcoded generic toasts → `extractApiError`; ConcludeModal swallowed failures and lost input; (6) `isLoading` without `keepPreviousData`; missing required asterisks (incident modal had none).
- **Commit**: `71d0e46` (error codes + en/vi.json keys landed earlier via concurrent sessions' shared-file commits, same cross-carry as F-012/F-016)
- **Affected features**: F-014 only — all changes feature-local; `lib/apiError.ts` imported, not modified.
- **Retest level**: 2
- **Result**: PASSED — migrator applied the new migration; api+frontend images rebuilt; food-poisoning e2e **9/9** (incl. 2 new negative suites: required-validation 400s with `validationErrors`, code-not-reused-after-delete); Domain.Tests 223/223 (5 new guard tests); Application.Tests 357/357; Vitest 6/6; browser screenshot pass (inline vi validation, asterisks, incident select, zero console errors).
- **Details**: Runtime probe artifacts (`NDTP-2026-0055`, `VND-2026-0031` empty records) deleted after proving L1. Debt recorded, not changed: Verify permission covers error-report Acknowledge+Respond; no attachments module; map caps at 500/source. Earlier browser 400s during probing were antiforgery (stale anonymous XSRF after API login), not a product bug — FE rotates via `initializeCsrf()`.

### 2026-07-28 — F-016 alerts-news production-readiness hardening (incl. org-scope leak via linked references)

- **Cause**: Full LOGIC/UI/UX audit of `/alerts-news` (real stack, commit-drift retest — page had been rewritten by the RowActions/page-size batches after the last verify). Found: (1) **`LinkedAlertIds`/`BusinessId` accepted unvalidated** — nonexistent IDs → FK-violation 500; cross-org IDs accepted, and unscoped `ToDtosAsync` lookups leaked other orgs' alert titles / business names → scope validation (`FoodSafe:Alert:0005`, `FoodSafe:News:0007`) + scoped lookups; (2) missing `[StringLength]` on 7 DTO fields → 500 instead of 400; (3) `AtpAlert.Update` dropped `Source` silently; (4) editor modals nulled `businessId`/`thumbnailStoragePath` on every edit; (5) RowActions confirm mutations swallowed failures (no error toast) → `extractApiError` everywhere; (6) no `keepPreviousData`/`isFetching`; (7) missing `scroll.x`+fixed actions (clipped at 1280), no column sorting (page missed by the `4a4af68` batch), no category filters, antd default "Trống" empty state, no `?tab=` sync; (8) spec drift: `name: "OK"` confirm + overflow-recall after RowActions consolidation.
- **Commit**: `4fa40d5` (shared files en/vi.json + FoodSafeDomainErrorCodes.cs carried concurrent sessions' additive keys, same cross-carry as F-012)
- **Affected features**: F-016 only — all changes feature-local; shared components used but not modified (B1 fixed at call sites, not in `RowActions`, precisely to avoid a Level-3 sweep).
- **Retest level**: 2
- **Result**: PASSED — api+frontend containers rebuilt from this tree; `alerts-news` e2e 8/8 ×2 (incl. 2 new negative suites proving the 500→403/400 fixes and the source round-trip); Domain.Tests AtpAlert 10/10; Application.Tests AlertsAndTesting contracts 37/37; Vitest 6/6; visual recheck desktop+mobile screenshots.
- **Details**: A mid-verify frontend image was stale because a concurrent session's broken in-flight file failed `npm run build` inside Docker — rebuilt the image from a clean `git worktree` (HEAD + alerts-news changes only) rather than touching their file. Business decisions deliberately left open: news recall has no reason (unlike alerts), alert-options offers all statuses for linking, no business-picker/thumbnail-upload widgets yet (values now preserved on edit).

### 2026-07-28 — F-012 export-food-certificates production-readiness hardening (incl. vi-VN culture 500)

- **Cause**: Real-browser vi-VN audit of `/export-food-certificates` found: (1) **500 on every create/update from a vi-VN browser** — `[Range(typeof(decimal), "0", "999999999.999")]` on `Quantity` parses its string limits with the request culture; vi-VN throws `FormatException` inside model validation until an en-US request warms the per-DTO validator cache (Playwright's en-US default is why E2E never caught it) → fixed with `ParseLimitsInInvariantCulture = true`; (2) editor allowed changing the owning business that `UpdateAsync` always rejects → select disabled on edit + name fallback option; (3) Excel export dropped `Sorting`; (4) missing `.ThenBy(Id)` paging tiebreaker; (5) API accepted default `0001-01-01` IssueDate → domain guard `FoodSafe:ExportFoodCertificate:0007`; (6) hardcoded toasts → `extractApiError`; (7) `isLoading` → `isFetching`; (8) case-sensitive certificate-number search → server-side uppercase; (9) `scroll.x` 1600 → 1300; (10) spec `name: "OK"` confirm click → `/^(Xóa|Đồng ý|OK)$/`.
- **Commit**: working tree after `819b803` (code commit follows; localization strings + error-code constants landed earlier inside a concurrent session's commit)
- **Affected features**: F-012 only (all changes feature-local; shared components used but not modified).
- **Retest level**: 2
- **Result**: PASSED — fresh api+frontend containers rebuilt from this tree; scripted vi-VN browser run 12/14 + the 2 toast-read misses re-proven via DOM MutationObserver probe (localized duplicate toast captured). Cold-cache vi-VN POST/PUT → 200. Direct-API issueDate guard → 403 code 0007. noperm 403 / anon 401. Cleanup verified (0 leftover test rows).
- **Details**: `Range(typeof(...), "...", "...")` with string limits existed nowhere else in the BE (grep-verified). During the audit the API log also showed 500s on `POST /api/v1/app/risk-analysis` and `POST /api/v1/app/inspection-result` from a concurrent session — different root causes, tracked by that session's features (F-013/F-018).
...
### 2026-07-28 — F-010 /eligibility-certificates production-readiness hardening (`e6ce3f7`)

- **Cause**: dedicated FE+BE deep review with a real-browser recon pass first (same recurring defect set as F-007/F-008/F-009/F-011): `extractApiError` for all 9 mutation toasts (duplicate number previously surfaced as ABP "internal error" text behind a generic "Kiểm tra dữ liệu" toast); vi/en localization added for `FoodSafe:EligibilityCertificate:0001–0005` + `EligibilityCertificateExport:TooLarge`; `CannotChangeBusiness` promoted from hard-coded `UserFriendlyException` to error code 0005; scope filter deduplicated into `EligibilityCertificateScope`.
- **Beyond the series**: recon found a **live orphaned certificate** (active cert `E2E-STT24-78125469/QN` pointing at a soft-deleted business — blank name on the admin list AND the public lookup). Root cause: `BusinessAppService.DeleteAsync` guard only checked products + self-declarations. Guard extended to eligibility/CFS/export certificates + product/advertisement registrations (`FoodSafe:Business:0010` message updated vi/en). Follow-on defect fixed the same run: certificate `DeleteAsync` refused to delete orphans (403 — scoped business lookup fails for a soft-deleted business); added a cleanup path that skips flag-sync for soft-deleted businesses while still denying out-of-scope ones. Orphan removed via UI and confirmed gone after reload.
- **UX/UI**: delete confirm names the certificate; customized empty states (filtered vs blank); business-select + DatePicker placeholders; "(Cơ sở đã xóa)" fallback (list/drawer/public); zero-padded dates via new shared `formatDate` (additive to `utils/format.ts`).
- **Change class**: Level 2 for F-010 + targeted Level 3 for F-006 (shared `BusinessAppService.DeleteAsync`). Spec updates for intended behavior: delete-guard message regex, lifecycle confirm-click `/^(Xóa|Đồng ý|OK)$/` (known vi_VN label pitfall), `licensing.ts` empty-state matcher widened to `/Trống|Không tìm thấy/` (helper shared by 5 licensing suites).
- **Result**: PASSED — eligibility specs **6/6** + `business-delete-guard` **2/2** + `businesses-verification` **6/6** at rebuilt stack (workers=1, no interception); sibling licensing verification suites (F-008/F-009/F-011/F-012) **20/20** after the shared-helper change; Vitest eligibility **4/4**; BE Application build 0 errors; FE `tsc` clean. F-010 returned to VERIFIED at `e6ce3f7`; F-006 re-stamped to `e6ce3f7`.
- **Note**: environment noise during recon — a parallel session's container rebuilds invalidated the audit harness session (antiforgery "different claims-based user" → empty 400s); reproduced as harness-only (real FE refreshes CSRF post-login in `authApi.login`). Not a product defect.

### 2026-07-28 — F-013 /inspection production-readiness hardening (DIRTY → VERIFIED)

- **Trigger**: dedicated FE+BE deep review of Thanh tra - Kiểm tra ATTP; F-013 was DIRTY from the sorting batch (`8be91bc`) and both legacy specs were red (confirm-button `OK` → vi_VN "Đồng ý" pitfall).
- **Recon method**: full code read + live probe script against the real stack (cookie login, XSRF, no interception) — every LOGIC defect below was reproduced against the running API before any fix.
- **LOGIC defects fixed**: (1) plan `UpdateAsync` silently dropped edits to existing items (notes/plannedDate/sequence) — new `UpdateBusinessDetails` domain path; (2) plan-linked results accepted a business that didn't match the plan item, completed the wrong item, and let Skipped items become Completed — new validations `0019` + `0007` + guard in `MarkCompleted`; plan/item pairing now both-or-neither (`0018`) matching DB `chk_ir_plan_tuple` (plan-only link previously 500'd on that constraint); (3) `RecalculateViolationState` destroyed the user-entered total fine (5,000,000 ₫ + one un-priced violation saved `null`) — explicit input now wins, itemised sum only fills a blank total; (4) 500s → 4xx for item-without-plan, negative fine, plan-only link; (5) `year` unvalidated (0 accepted) → `[Range(2000,2100)]`; (6) future inspection dates accepted → rejected (`0020`) + FE `disabledDate`; (7) both Excel exports dropped `Sorting` (F-008/F-009 defect class); (8) `ApplySorting` tiebreaker `.ThenBy(Id)` (F-008 precedent); (9) `FoodSafe:Inspection:0016/0017` unlocalized (raw code shown on finalized-result errors) — added with new `0018/0019/0020` (vi+en, additive edits to files a parallel session also had dirty).
- **UX/UI**: extractApiError across ~15 toasts (page + 4 modals); result editor disables plan-link fields on edit (server ignores changes) and auto-fills business from the chosen plan item (+name-fallback options); plan editor gained per-item "Ngày dự kiến" and client-side empty/duplicate row validation; `keepPreviousData`+`isFetching`; year filter commits on Enter/blur; DD/MM/YYYY dates; "Bị từ chối" tag for rejected drafts; fine formatter+parser; checkbox alignment; token colors.
- **Change class**: Level 2 (all runtime changes inside the Inspection module; shared-file edits are additive constants/localization keys only — no Level 3 dependencies touched).
- **Result**: PASSED — inspection Playwright **11/11** at rebuilt api+frontend images (workers=1, no interception, `locale: "vi-VN"`); live probe script 15/15 API checks + UI screenshots; Vitest inspection **5/5**; BE Application compile 0 errors; FE `tsc -b` + `oxlint` clean. Domain.Tests could not run at fix time (a parallel session's WIP broke unrelated AlertsAndTesting test compilation); inspection behavior evidence is the real-API probe set per policy. F-013 returned to VERIFIED.
- **Deliberately unchanged (business rulings pending)**: "Đạt" may coexist with "Có vi phạm"; remediation/follow-up remain editable after finalize; soft-deleted plans still block planCode reuse.
