...
| FR-33-02 | NDTP roll-up aggregation | `ndtp-rollup-aggregation` ✓ (714e318) | pass | PASS_WITH_BROWSER_EVIDENCE — higher-tier officer clicks "Tổng hợp từ báo cáo tuyến dưới" in the draft editor; real-HTTP seed of a Submitted lower-tier report, `GET /report-calculation/ndtp-aggregation` delta = baseline + child reportCount and all 8 stat fields, org-isolation guard, form fields populated with aggregated values, saved + persists after browser reload; no API interception |
| FR-34-01..07/09/11 | ATTP work report CRUD + workflow | `reporting-verification` ✓, `reporting` ✓ | pass | PASS_WITH_BROWSER_EVIDENCE |
| FR-34-08 | ATTP formatted view | — | none | IMPLEMENTED_NOT_VERIFIED |
| FR-34-10 | ATTP work-report auto-aggregation | `atp-work-auto-aggregation` ✓ (7d38721) | pass | PASS_WITH_BROWSER_EVIDENCE — officer clicks "Tự tính số liệu từ hệ thống" in the ATTP draft editor; real-HTTP seed of one business increments `GET /report-calculation/atp-work-stats` totalBusinesses/newBusinesses by exactly one (baseline-delta, non-zero real figures), browser click populates the form fields with EXACTLY the endpoint response, saved + persists after full page reload; no API interception |
...
| FR-38 (attachments) | Document file attachments up/list/download/delete | `documents-attachments` ✓ (9ca9a7b) | pass | PASS_WITH_BROWSER_EVIDENCE — real `/documents` attachment modal round-trip: upload a real PDF (real multipart → ClamAV → MinIO → PostgreSQL), row shows real "N KB" size + valid date, persists after full reload, downloads with "%PDF" magic, delete removes the row. **Fixed a real FE defect**: the modal read `sizeBytes`/`creationTime` (absent from the DTO → NaN KB / Invalid Date) instead of the DTO's `fileSize`/`uploadTime`. No API interception |
...
| FR-39-03/04 | Dashboard per-unit report-compliance widget (ATTP work + Action-Month submission tracking) | `dashboard-report-compliance` ✓ (P1-1j) | pass | PASS_WITH_BROWSER_EVIDENCE — one ATTP work report + one Action-Month report are seeded and submitted for the officer's org via the real workflow; the compliance endpoint then shows +1 for each, and the real `/dashboard` widget renders the org's "BC công tác ATTP" (1/2) and "BC Tháng hành động" (1/1) progress cells, persisting across reload |
| FR-39-09 | Statistics chart PNG download | `statistics-chart-download` ✓ (de06374) | pass | PASS_WITH_BROWSER_EVIDENCE — real `/statistics` browser test clicks both time-series download buttons ("Thanh kiểm tra theo tháng" + "Ngộ độc thực phẩm theo tháng") and asserts real PNG downloads (8-byte PNG signature + year-stamped filenames `thanh-kiem-tra-<year>.png` / `ngo-doc-thuc-pham-<year>.png`). **Fixed a real production defect**: `downloadChartAsPng` rasterized the SVG through a `blob:` URL, which the app CSP (`img-src ... data:`, no `blob:`) rejected before load — the export silently failed; switched to a CSP-allowed `data:` URI. No API interception |
...
## 9. Addendum — Batch F-1: typed share payloads + retry attempt history (FR-51..57) executed

`data-integration-retry.spec.ts` — **3/3 green (6.4s)**, real rebuilt stack (migration `20260728001241` applied), no interception. Full detail: `docs/functional-audit/03` + `04`.

- **FR-51..57-02 (typed payload)** PARTIAL → **PASS_WITH_BROWSER_EVIDENCE (transport + real data)**: the share envelope now carries the REAL records of the selected type via per-type payload builders (`schemaVersion 1.0`, `recordCount`, `records[]`, camelCase + string enums). Proven by parsing the logged request body of a share pinned to a seeded alert (`recordCount=1`, exact title/id) and by the receiver's reflection containing the record. Per-partner TT 31/2026 field mapping remains blocked-external (INT-02 remainder).
- **FR-51..57 Retry ("Thử lại")** MISSING → **PASS_WITH_BROWSER_EVIDENCE**: failed outbound rows expose a Share-permission-gated retry button; retry re-sends the stored body verbatim and appends an immutable linked attempt (`correlationId` = original, `attemptNumber` 2, identical SHA-256 `payloadChecksum`), original row untouched; persists after full reload. Guards: successful attempt → 403 VN error; readonly → 403.
- **FR-51..57-03 (time filter)**: new date-range filter browser-verified (past-only range empties the table).
- **Attempt-history requirement (docs/01 §STT 51–57)**: `di_api_call_logs` + `endpoint_id`, `correlation_id`, `attempt_number` (check ≥ 1), `payload_checksum` — every attempt an immutable row.
- **F-019c re-verified** after this batch: the typed payload enlarged the echo reflection beyond the 4000-char response truncation, hiding the reflected auth headers the credentials spec asserts on; its share setup now pins one seeded record (`entityId`) — assertions unchanged — **6/6 green**. DataIntegration subset **20/20**; cross-module smoke (businesses + auth) **13/13**.
